"""GAT model with Digital Twin awareness (7 input features)."""
...
class GAT_DTAware(nn.Module):
    """
    GAT model with Digital Twin awareness.
    ...
    def forward(self, state):
        ...
        for batch in loader:
            ...
            out = self.conv1(batch.x, batch.edge_index, edge_attr=edge_attr)
            out = F.selu(out)
            out = self.conv2(out, batch.edge_index, edge_attr=edge_attr)
...
class GAT_DTAgent:
    """
    DQN agent using GAT_DTAware model.
    
    Epsilon-greedy exploration with experience replay.
    """
    
    def __init__(self, num_nodes: int, policy_net: GAT_DTAware, 
                 target_net: GAT_DTAware, env):
        """
        Args:
...
if __name__ == "__main__":
    print("Testing GAT_DTAware model...")
    
    model = GAT_DTAware(num_nodes=50, out_dim=50, node_feat_dim=7)
    ...
    edge_attr = torch.randn(100, 1)  # DT-derived edge weights
    ...
    print("edge_attr passed to GATConv: OK")
