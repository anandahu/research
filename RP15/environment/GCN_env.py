...
#         f.close()
    ...
    def step(self, action: int):
        ...
        node_data = self.compute_node_features()
        return Data(x=node_data, edge_index=self.edge_list), rewards[0], done, {'valid_actions': valid_actions}
    ...
    def _reset(self) -> Tuple[int, int]:
        ...
        if self.eps % 50 == 0:
            ...
            self.latency_record = []
        node_data = self.compute_node_features()
        return Data(x=node_data, edge_index=self.edge_list), {'valid_actions': valid_actions}
    ...
    def compute_node_features(self):
        x = torch.zeros((self.num_nodes_in_graph, NODE_FEATURE_DIM), device=device)

        for node in self.graph.nodes():
            neighbors = list(self.graph.neighbors(node))
            if neighbors:
                lat = sum(self.graph[node][n]['weight'] for n in neighbors) / len(neighbors)
                bw  = sum(self.graph[node][n]['capacity'] for n in neighbors) / len(neighbors)
            else:
                lat, bw = 0.0, 0.0
        ...
        x[self.current_node, 0] = 1.0   # source
        x[self.target, 1] = 1.0         # destination
