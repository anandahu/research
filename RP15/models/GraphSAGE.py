...
class GraphSAGEPolicy(nn.Module):
    def __init__(self, num_nodes, out_dim, node_feat_dim):
        super().__init__()

        h1 = num_nodes // 3
        h2 = num_nodes // 2

        self.conv1 = SAGEConv(node_feat_dim, h1)
        self.conv2 = SAGEConv(h1, h2)

        self.fc1 = nn.Linear(num_nodes * h2, num_nodes)
    ...
    def forward(self, state):
        ...
        for batch in loader:
            assert batch.x.shape[1] == 4, f"Expected 4 node features, got {batch.x.shape[1]}"
            x = self.conv1(batch.x, batch.edge_index)
            x = F.relu(x)
            x = self.conv2(x, batch.edge_index)
            x = F.relu(x)

        x = torch.stack(x.split(self.num_nodes)).contiguous()
        x = x.view(x.size(0), -1)
        ...
        x[~mask] = float('-inf')   # action masking (unchanged)
        return x
